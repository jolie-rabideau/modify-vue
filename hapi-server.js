...

    {
      ...
      handler: async (request, h) => {
        const existingAccount = await Account.query()
          ...
          .first();
          if (existingAccount && 
             (await existingAccount.verifyPassword(request.payload.currentPassword)) &&
             request.payload.newPassword == request.payload.confirmPassword
          ) {
            return {
              ok: true, 
              msge: `'${request.payload.email}' password reset successfully`,
              details: {
